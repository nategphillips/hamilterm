...
function three_sigma(num::Int)
    c = AllConsts(
        ...
        spin_spin=SpinSpinConsts(λ=1.69),
        spin_rotation=SpinRotationConsts(γ=-0.028)
    )
    for _ in 0:num
...
function five_pi(num::Int)
    c = AllConsts(
        ...
        spin_orbit=SpinOrbitConsts(A=-139.21, A_D=1.0, A_H=1.0, A_L=1.0, A_M=1.0, η=1.0),
        spin_spin=SpinSpinConsts(λ=1.69, λ_D=1.0, λ_H=1.0, θ=1.0),
